...
function entropy(X)
    ...
    b = count(X.Survived .== 1)  # Survived
    a = count(X.Survived .== 0)  # Died
...
function getGoodGain(complete_df,colname,type::String)
    ...
    if type=="c"
        ...
        for cat in unique_cats
            ...
            survival_rate = nrow(subset) > 0 ? count(subset.Survived .== 1) / nrow(subset) : 0.0
...
function maketree(complete_df,column_types)
    ...
    if isempty(column_types) || nrow(complete_df) == 0
        survived_count = count(complete_df.Survived .== 1)
        died_count = count(complete_df.Survived .== 0)
    ...
    if colbest === nothing ||colbest ==="Survived"
        survived_count = count(complete_df.Survived .== 1)
        died_count = count(complete_df.Survived .== 0)
        leaf_value = survived_count >= died_count
        return node(leaf_value)
...
function main()
    ...
    df = CSV.read("titanic.csv",header=true, DataFrame)
    ...
    column_types = Dict(
    "Survived" => "b",  # Includes target variable!
    "Pclass" => "c",
    "Sex" => "b", 
    "SibSp" => "c",
    "Parch" => "c", 
    ...
    split_idx = round(Int, 0.9 * nrow(complete_df))
    training_data = complete_df[1:split_idx, :]
    root=maketree(training_data,column_types)
    println("\nDecision Tree:")
    show(stdout, root, 0)
    println()
    estimatingfield="Survived"
   
    test_data=vcat(last(complete_df[((complete_df[!, estimatingfield]) .== 1), :], 30)
                            ,last(complete_df[((complete_df[!, estimatingfield]) .!= 1), :], 30))
    actualsurvived=0
    actualdied=0
    correct=0
    incorrect=0
    for x in eachrow(test_data)
        #println(x)
        actualsurvived+=x["Survived"]==true
        actualdied+=x["Survived"]==false
        correct+=x["Survived"]==predict(root,x)
        incorrect+=x["Survived"]!=predict(root,x)
    end
    println(actualsurvived)
    println(actualdied)
    ic=(correct/(nrow(test_data)))
    iw=(incorrect/(nrow(test_data)))
    println(ic)
    println(iw)
    println(ic+iw)
    #println(prediction)
